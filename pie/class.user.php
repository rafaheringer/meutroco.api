...
updateSessionObj();
	...
	public function groupID()
	{
		...
		$sql = "SELECT ".$db_table_prefix."profiles.group_id, 
			   ".$db_table_prefix."groups.* 
			   FROM ".$db_table_prefix."profiles
			   INNER JOIN ".$db_table_prefix."groups ON ".$db_table_prefix."profiles.group_id = ".$db_table_prefix."groups.group_id 
			   WHERE
			   id  = '".$db->sql_escape($this->id)."'";
	...
	public function isGroupMember($id)
	{
		...
		$sql = "SELECT ".$db_table_prefix."profiles.group_id, 
				".$db_table_prefix."groups.* FROM ".$db_table_prefix."profiles 
				INNER JOIN ".$db_table_prefix."groups ON ".$db_table_prefix."profiles.group_id = ".$db_table_prefix."groups.group_id
				WHERE id  = '".$db->sql_escape($this->id)."'
				AND
				".$db_table_prefix."profiles.group_id = '".$db->sql_escape($db->sql_escape($id))."'
				LIMIT 1
				";
